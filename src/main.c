...
int main(void)
{
    ...
    char *destLabel = (char *)RL_CALLOC(4096, 1);

    TextCopy(csvFilePath, "Drop a CSV File Here");
    TextCopy(srcLabel, theGraph.labels[0]);
    TextCopy(destLabel, theGraph.labels[theGraph.n - 1]);
        ...
        switch (currentScene)
        {
            ...
            case MAIN_SCENE:
            {
                ...
                if (IsFileDropped())
                {
                    ...
                    if ((droppedFiles.count > 0) && IsFileExtension(droppedFiles.paths[0], ".csv") && !dijkstraActive && csvStatus != IN_PROGRESS)
                    {
            ...
            switch (currentScene)
            {
                ...
                case MAIN_SCENE:
                {
                    mainScene(
                        ...
                        &dijkstraData, &adjacencyMatrixWindowActive, &edgeThickness, debugInfoActive
                    );
                    break;
                }
                default:
...
//----------------------------------------------------------------------------------
void mainScene(
    Vector2 *points, Graph *theGraph, int pointCount, int *focusedPoint, SceneType *currentScene,
    dijkstraThreadData *dijkstraData, bool *adjacencyMatrixWindowActive, float *edgeThickness, bool debugInfoActive
)
{
    // Draw Edges
    for (int i = 0; i < pointCount; i++)
    ...
    // Draw Algorithm Source and Destination Labels
    ...
    if (*(dijkstraData->animationActive) && *(dijkstraData->status) == IN_PROGRESS)
    {
        // Draw Title
    ...
    if (*adjacencyMatrixWindowActive)
    {
        ...
        *adjacencyMatrixWindowActive = !GuiWindowBox((Rectangle){windowX, windowY, windowSize - 16, windowSize}, "Adjacency Matrix");
        for (int i = 0; i < pointCount; i++)
        {
            for (int j = 0; j < pointCount; j++)
            {
                if (CheckCollisionPointRec(GetMousePosition(), (Rectangle){windowX + 40 + 50 * j, 72 + 50 * i, 40, 40}) && !(*(dijkstraData->animationActive)))
                {
